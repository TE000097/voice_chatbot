...
def trigger_start_call():
    ...
    payload = {
        "customer_name": selected["Debtor_Name"],
        "system_id": selected["system_id"],
        "loan_id": selected["Loan_ID"],
        "due_date": selected["Payment_Due_Date"],
        "due_amount": float(selected["EMI_Amount"]),
        "product": selected["Product"],
    }

    try:
        response = requests.post(f"http://{url}/start-call", json=payload)
...
async def on_audio_start():
    try:
        ...
        audio_element = cl.Audio(name="introvoicebot16k.wav", 
                                 path="./introvoicebot16k.wav",
                                #  type="audio/wav", 
                                 display="inline", 
                                 auto_play=True)
...
async def on_audio_chunk(audio_chunk: cl.InputAudioChunk):
    try:
        ...
        if ws_client and ws_client.connected:
            ...
            await cl.Message(content="WebSocket not connected.").send()
            return False
    except Exception as e:
